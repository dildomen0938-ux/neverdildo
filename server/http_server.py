...
HOST = "0.0.0.0"
PORT = int(os.environ.get("PORT", 30031))  # Railway использует переменную PORT

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(message)s", datefmt="%H:%M:%S")
log = logging.getLogger("http")

DATA_DIR = Path(__file__).parent / "data"
DLL_FILE = DATA_DIR / "neverlose.dll"
...
class NLRequestHandler(BaseHTTPRequestHandler):
    ...
    def _route(self, method):
        ...
        if path == "/api/config":
            ...
            railway_host = os.environ.get("RAILWAY_PUBLIC_DOMAIN", "localhost")
            self._send_json(200, {
                "status": "ok",
                "version": "2.0",
                "update": False,
                "server_host": railway_host,  # Адрес Railway хоста
                "wss_port": 443,  # Railway использует HTTPS
                "http_port": 443,
                "config": {
                    "glow": True,
                    "esp": True,
        ...
        # =====================================================================
        # Route: /download/neverlose.dll
        # Loader downloads DLL from Railway host
        # =====================================================================
        if path == "/download/neverlose.dll":
            log.info("    -> Download DLL request")
            if DLL_FILE.exists():
                dll_data = DLL_FILE.read_bytes()
                log.info(f"    -> Serving DLL ({len(dll_data):,} bytes)")
                self._send_binary(200, dll_data, "application/octet-stream")
            else:
                log.error("    -> DLL not found!")
                self._send_json(404, {"error": "DLL not found"})
            return
...
def main():
    ...
    if os.environ.get("RAILWAY_PUBLIC_DOMAIN"):
        log.info(f"  Railway URL: https://{os.environ.get('RAILWAY_PUBLIC_DOMAIN')}/")
    log.info("")
    ...
    log.info("  GET /download/neverlose.dll - Download DLL")
    log.info("")
